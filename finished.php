...
<!--内容-->
    <div class="container" style="padding-top:200px;">
     <div class="btn-group" style="margin: 0 auto;" >
         <a href="tea_unfinished.php?p=1"><button type="button" class="btn btn-default  btn btn-sm ">教学区任务板块</button></a>
         <a href="unfinished.php?p=1"><button type="button" class="btn btn-default btn btn-sm  btn-mybt ">生活区任务板块</button></a>
     </div>
    </div>    
    <div class="container" style="padding-top:50px;width:auto">
	   ...
	   <table class="imagetable" align="center">
		<p align="center"><br><br>点击<a href="unfinished.php?p=1">学生未处理的任务</a>学生查看未完成的任务。</p>
		<td colspan="11" align="center"><font size="3">学生已处理任务</font></td>
		<tr>
		  ...
		  $page=$_GET['p'];
		   ...
		   mysqli_query($conn,"SET NAMES UTF8");
			...
			$total_sql="SELECT COUNT(*)FROM student where state='已处理'";
			$total_result=mysqli_fetch_array(mysqli_query($conn,$total_sql));
			$total=$total_result[0];
			$total_pages=ceil($total/$pageSize);
			
			if($page<$total_pages)
			$sql="SELECT * FROM student order by state ASC,finish_time ASC LIMIT ".($page-1)*$pageSize.",{$pageSize}";
			else		 
			{   
				...
				{
					$x=$total%$pageSize;
				    //echo"$x,$total,$pageSize";
			        $sql="SELECT * FROM student order by state ASC ,finish_time ASC LIMIT ".($page-1)*$pageSize.",{$x}"; //最后一页特殊情况取余
				}
		   ...
		   //echo "<table border=1 cellspacing=0 width=auto align=center>";
		   while ($row = mysqli_fetch_assoc($result)) {
		        ...
		        echo "<td><a href='#' class='btn btn-sm btn-success disabled' > 已处理</a></td>";
		        echo "</tr>";
		   }
		   echo "</table>";
		   echo "</div>";
		  ...
		  /*------分页部分-------*/
		   ...
		   $page_banner .= "<form action='finished.php' method='get' class='form-inline my-2 my-lg-0'>";
