...
<!--内容-->
   <div class="container" style="padding-top:200px;">
    ...
    <div class="container" style="padding-top:50px;width:auto">
		...
		<p align="center"><br>任务完成后请点击对应任务的完成按钮<br>点击<a href="finished.php?p=1">已处理的任务</a>查看生活区已完成的任务。</p>
	        <td colspan="11" align="center"><font size="3">生活区未处理任务</font></td>
			<tr>
				<th>姓名</th>
				<th>学号</th>
				<th>班级</th>
		    ...
		    <?php
		     ...
		     $page=$_GET['p'];
		      ...
		      //编写sql获取分页数据 SELECT * FROM 表名 LIMIT 起始位置，显示条数
		     	$qid = mysqli_query($conn,"SELECT count(*) as total FROM student WHERE state='已处理'");//查询已处理条数
		     	$res = mysqli_fetch_array($qid);
		     	$done = $res['total'];
		     	$sql="SELECT * FROM student order by state ASC ,number ASC LIMIT ".(($page-1)*$pageSize+$done).",{$pageSize}";
		      $result=mysqli_query($conn,$sql);//把sql语句传送到数据库
		      /*-----表格部分-----*/
		      //echo "<div class='conntent'>";   //将数据显示到table中，并未table设置格式
		      //echo "<table border=1 cellspacing=0 width="auto" >";
		      while ($row = mysqli_fetch_assoc($result)) {
		           ...
		           echo "<td><a href='operate.php?aim=".$row['tel']."'><div class='btn btn-sm btn-mybt '>完成</div></a></td>";
		           echo "</tr>";
		      }
		      echo "</table>";
		      echo "</div>";
		      ...
		      $page_banner .= "<form action='unfinished.php' method='get' class='form-inline my-2 my-lg-0'>";
